...
if (__name__ == '__main__'):
    ...
    data = []
    path = '/home/mingwu/workspace_czg/data/LRW/LRW/images/LRW1000_Public/images'
    for dir_s in [p for p in os.listdir(path)]:
        cur_path = os.path.join(path, dir_s)
        for file in [p for p in os.listdir(cur_path) if p.endswith('.jpg')]:
            image_name = os.path.join(cur_path, file)
            if (os.path.exists(image_name.replace('.jpg', '.txt'))):
                data.append(image_name)

    data = list(set([os.path.split(file)[0] for file in data]))

    annos = [name for name in data]
    targets = [name.replace('/home/mingwu/workspace_czg/data/LRW/LRW/images/LRW1000_Public/images',
                            '/home/mingwu/workspace_czg/data/LRW/LRW/images/LRW1000_Public/lip') for name in data]
